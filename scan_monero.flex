...
#include "extern/Keccak-more-compact.c"

#define FEATURE_NAME "monero"

class monero_scanner : public sbuf_scanner {
public:
    monero_scanner(const scanner_params &sp):
        sbuf_scanner(&sp.sbuf),
        monero_recorder(sp.fs.get_name("monero")),
...
{START_STD_SUB}{BLOCK_STD_SUB} {
    ...
    if(check_xmr(yytext,yyleng, XMR_STD_SUB)){
        s.monero_recorder->write_buf(SBUF,s.pos,yyleng);
    }    
    s.pos += yyleng;
...
extern "C"
void scan_monero(const class scanner_params &sp,const recursion_control_block &rcb) {
