...
{BASE32}{56}{TLD} {
    ...
    if(validate_torurl(yytext)){
        s.torurls_recorder->write_buf(SBUF,s.pos,yyleng);
    }
    s.pos += yyleng; 
}
...
extern "C"
void scan_torurls(const class scanner_params &sp,const recursion_control_block &rcb) {

    assert(sp.sp_version==scanner_params::CURRENT_SP_VERSION);
    if ( sp.phase==scanner_params::PHASE_STARTUP ) {
        assert(sp.info->si_version==scanner_info::CURRENT_SI_VERSION);
        sp.info->name           = "torurls";
