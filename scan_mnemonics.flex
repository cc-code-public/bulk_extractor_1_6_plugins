...
{DB_BIP39}{11,}{BLOCK_BIP39} {
    ...
    if ( cb39 == 0 ){
        ...
        if (modify_scanner == 0) {
            ...
            if (HMAC_SHA_512(yytext_lower, yyleng) || libbitcoin::system::wallet::validate_mnemonic(split(yytext_lower, ' '), language::en)) {
                if (HMAC_SHA_512(yytext_lower, yyleng)) {
                    //~ tue etwas
                }
                
                if (libbitcoin::system::wallet::validate_mnemonic(split(yytext_lower, ' '), language::en)) {
                    //~ tue etwas
                }

                s.mnemonics_recorder->write_buf(SBUF,s.pos, yyleng);
                s.pos += yyleng;
            } else {
                s.pos++;
                yyless(1);
            ...
            //~ treffer sollte eigentlich nicht größer als yyleng sein, aber wenn doch, nicht weiter gehen
                ...
                if (HMAC_SHA_512(argv, modify_scanner) || libbitcoin::system::wallet::validate_mnemonic(split(argv, ' '), language::en)) {
                    if (HMAC_SHA_512(argv, modify_scanner)) {
                        //~ tue etwas
                    }
                    if (libbitcoin::system::wallet::validate_mnemonic(split(argv, ' '), language::en)) {
                        //~ tue etwas
                    }
                    s.mnemonics_recorder->write_buf(SBUF,s.pos, modify_scanner);
...
extern "C"
void scan_mnemonics(const class scanner_params &sp,const recursion_control_block &rcb) {

    assert(sp.sp_version==scanner_params::CURRENT_SP_VERSION);
    if ( sp.phase==scanner_params::PHASE_STARTUP ) {
        assert(sp.info->si_version==scanner_info::CURRENT_SI_VERSION);
        sp.info->name           = "mnemonics";
        sp.info->author         = "Christian C., https://github.com/cc-code-public";
        sp.info->description    = "Scans for mnemonics";
        sp.info->scanner_version= "0.9";
